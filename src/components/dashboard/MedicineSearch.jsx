        ...
        {result && result.medicines && (
          ...
          >
            {result.medicines.map((medicine, index) => (
              <Card key={index} className="p-8 border-brand-cyan/30 bg-brand-navy/60 backdrop-blur-md">
                <div className="flex items-start justify-between border-b border-white/10 pb-6 mb-6">
                  <div className="flex items-center space-x-4">
                    ...
                    <div>
                      <h3 className="text-3xl font-syne font-bold text-white">{medicine.name}</h3>
                      <p className="text-white/60 font-medium">{medicine.generic_name} • {medicine.category}</p>
                    </div>
                  </div>
                </div>
                  ...
                  <div className="space-y-6">
                    ...
                    <div>
                      ...
                      <ul className="space-y-2">
                        {medicine.common_uses.map((use, i) => (
                      ...
                      <div className="bg-amber-400/5 border border-amber-400/20 p-4 rounded-xl space-y-4">
                        ...
                        {medicine.side_effects.length > 0 && (
                          <div>
                            ...
                            <p className="text-sm text-white/70">{medicine.side_effects.join(', ')}</p>
                          </div>
                        )}
                      </div>
                    </div>
